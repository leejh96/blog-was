...
import { duplicatedData, isNotAuthUser, serverError } from 'share/error-msg/server';
    ...
    if (exception instanceof HttpException) {
      ...
      if (typeof exceptionResponse === 'object' && 'message' in exceptionResponse) {
        if (Array.isArray(exceptionResponse.message)) {
          ...
          // JWT Error 처리
          failRes.message = isNotAuthUser;
        }
      } else {
        failRes.message = exception.message || 'HTTP Exception occurred';
      }
      ...
      if (exception.code === 'P2002') {
        ...
        failRes.message = duplicatedData(exception.meta.modelName, exception.meta.target);
