// 성공 메시지
export const successMsg = '요청에 성공하였습니다.';
export const failMsg = '요청에 실패했습니다.';
// 에러 메시지
export const invalidPassword =
    '비밀번호는 최소 8자리, 최대 20자리 이며 영문자와 숫자를 조합해야 합니다.';
export const invalidEmailForm = '올바른 이메일 형식을 입력하세요.';
export const invalidString = (field: string) =>
    `${field}이(가) 문자열이 아닙니다.`;
export const invalidLength = (field: string, min: number = 1, max: number) =>
    `${field}은(는) 최소 ${min}, 최대 ${max}자까지 입력 가능합니다.`;
