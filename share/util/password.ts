...
export class PasswordService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.saltRounds = parseInt(this.configService.get<string>('BCRYPT_SALT_ROUNDS', '10'));
  }

  async hashPassword(password: string): Promise<{ success: boolean; password?: string }> {
    try {
